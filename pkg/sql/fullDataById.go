...
(
	...
	(array_agg(distinct stc.rockTypeObj) filter (where stc.rocktypeobj <> '{}')) as rock_types,
	(array_agg(distinct stc.rockClassObj) filter (where stc.rockclassobj <> '{}')) as rock_classes,
